...
def load_example(topo_path, data_path):
    ...
    for path in data["paths"]:
        shortest_path = len(path[0]) <= len(
            nx.shortest_path(topo, source=path[0][0], target=path[0][-1])
        )
        paths.append(
            NDBEntry(path[0], path[1], path[2], shortest_path, path[4], path[3])
        )

    return (
        paths,
        topo,
        destination_to_prefix,
        prefix_to_destination,
        node_to_name,
        name_to_node,
    )
...
def main(example_path):
    ...
    output += "There is a total of {} flows in a topology with {} nodes and {} edges.\n\n".format(
        len(paths), len(topo.nodes()), len(topo.edges())
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "path", help="path to the directory containing the example", type=str
    )
